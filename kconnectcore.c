...
void print_header(void) {
    printf("%s%-8s %-16s %-6s %-20s %-7s %-7s%s\n",
           CLR_BOLD,
           "PID", "PROCESS", "PROTO", "DEST IP", "DPORT", "SPORT",
           CLR_RESET);
...
void print_connection(const Connection *c) {
    const char *proto_color = (strncmp(c->proto, "TCP", 3) == 0)
                              ? CLR_CYAN : CLR_MAGENTA;
    ...
    printf("%-8d "                          
           ...
           "%s%-7d%s\n",                    
           c->pid,
           CLR_DIM, c->comm, CLR_RESET,
           proto_color, c->proto, CLR_RESET,
           c->daddr,
           port_color, c->dport, CLR_RESET,
           CLR_DIM, c->sport, CLR_RESET);
}
...
void print_summary(void) {
    ...
    for (int i = 0; i < top_n; i++) {
        printf("%s%s%s %s%d%s  ",
               CLR_CYAN, proc_names[top_idx[i]], CLR_RESET,
